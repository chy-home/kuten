...
if [[ ! -x "$VENV_PYTHON" ]]; then
  ...
  echo "Run: python3 -m venv .venv && .venv/bin/pip install numpy scenedetect opencv-python" >&2
  exit 2
fi

if ! "$VENV_PYTHON" -c "import numpy, scenedetect, cv2" >/dev/null 2>&1; then
  echo "The virtualenv is missing required detector dependencies." >&2
  echo "Run: .venv/bin/pip install numpy scenedetect opencv-python" >&2
  exit 2
fi
