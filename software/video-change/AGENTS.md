...
- macOS app runtime:
  ...
  - the app should use a Python interpreter that can actually import detector dependencies, not silently fall back to a bare system Python
- Python runtime:
  - `.venv/bin/python`
...
- `build_macos_app.sh` should fail early if `.venv/bin/python` is missing
- `build_macos_app.sh` should fail early if `numpy`, `scenedetect`, or `opencv-python` are not importable in `.venv`
- the bundled `Resources/venv` symlink is expected to point at the project-root `.venv`
...
- RuntimeResolver should only accept a Python interpreter that can import `numpy`, `scenedetect`, and `cv2`.
- Do not reintroduce a silent fallback to `/usr/bin/python3` when that interpreter lacks detector dependencies.
...
## 重新编译
- 1. 重建 .venv：python3 -m venv .venv && .venv/bin/pip install numpy scenedetect opencv-python
